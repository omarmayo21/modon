$(document).ready(function () {
	...
	$("#generic-project-form").validate({
		...
		messages: {
			...
			'00NHp00000rketg': $("#PropertyTypeErrorMessage").val(), // "Please select property type.",
			'00NHp00000lQiUS': $("#BudgetErrorMessage").val(), //"Please select budget."
			consentCheck: ''
		}
	});
	...
	//	$(this).inputmask({ "mask": "\\+999999999999", "placeholder": "" });
	//});
	...
	$("#first_name,#last_name,#country").on('input', function () {
		$(this).val($(this).val().replace(/[^A-Za-z\u0600-\u06FF\u0750-\u077F+\s]/g, ''))
	});
...
$('#SFFormSubmit').on('click', function () {
	if ($('#PhoneNumber').valid()) {
		...
		var phoneNumberValue = $("#generic-project-form").find('.iti__selected-dial-code').text() + $("#generic-project-form").find("#PhoneNumber").val();
